...
INSERT INTO card_categories (name, description, icon, sort_order) 
SELECT * FROM (VALUES
...
WHERE NOT EXISTS (SELECT 1 FROM card_categories WHERE card_categories.name = v.name);

-- Insert default sharing templates
INSERT INTO sharing_templates (name, description, template_permissions, is_public, created_by) 
SELECT * FROM (VALUES
  ('Basic View', 'Basic information viewing permissions', '{"cards": ["view_basic"], "profile": ["view_basic"]}'::jsonb, true, NULL::uuid),
  ('Full Access', 'Complete access to all information', '{"cards": ["view_basic", "view_detailed", "edit"], "profile": ["view_basic", "view_detailed"]}'::jsonb, true, NULL::uuid),
  ('Organization Member', 'Standard permissions for organization members', '{"cards": ["view_basic", "view_detailed"], "profile": ["view_basic"], "organization": ["view_basic"]}'::jsonb, true, NULL::uuid),
  ('Service Provider', 'Permissions for service providers', '{"cards": ["view_basic", "view_detailed"], "profile": ["view_basic"], "services": ["read", "write"]}'::jsonb, true, NULL::uuid)
) AS v(name, description, template_permissions, is_public, created_by)
WHERE NOT EXISTS (SELECT 1 FROM sharing_templates WHERE sharing_templates.name = v.name);